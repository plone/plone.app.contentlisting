...
class TestIndividualContentItems(ContentlistingFunctionalTestCase):
    ...
    def afterSetUp(self):
        ...
        should be done in that test method.
        """
        new_id = self.folder.invokeFactory('Document', 'my-page', title='My Page', description='blah')
    ...
    def test_item_getId(self):
        ...
        self.assertEqual(self.item.getId(),'my-page')

    def test_item_getIcon(self):
        """checking the getId method"""
        self.assertEqual(self.item.getIcon(),u'<img width="16" height="16" src="http://nohost/plone/document_icon.png" alt="Page" />')
...
class TestSearch(ContentlistingFunctionalTestCase):
    ...
    def test_search_for_pages(self):
        ...
        searchresultslist = self.folder.restrictedTraverse('@@searchResults')(Type="Page")
        self.assertEqual(len(searchresultslist), 3)

    def test_search_with_batching(self):
        """Check that batching works"""
        searchresultslist = self.folder.restrictedTraverse('@@searchResults')(batch=True, b_size=1, Type="Page")
        self.failUnless(verifyObject(IContentListing, searchresultslist))
    ...
    def test_search_with_batching_2(self):
        ...
        # we make 2 queries, one starting at the second batch. Test to make sure we don't get the same result from both queries
        searchresultslist = self.folder.restrictedTraverse('@@searchResults')(batch=True, b_size=1, b_start=1, Type="Page")
        firstbatchitem = searchresultslist[0].getId()
        searchresultslist = self.folder.restrictedTraverse('@@searchResults')(batch=True, b_size=1, b_start=2, Type="Page")
